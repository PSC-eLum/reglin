...
os.chdir('C:\Users\damien\Documents\PSC\Data')
...
size = len(pp1)
X_training = np.array(pp1[:]).astype(np.float)
X_training = np.reshape(X_training,(len(X_training),1))

Y_training = np.array(gen[:]).astype(np.float)
Y_training = np.reshape(Y_training,(len(Y_training),1))

X_test = np.array(pp1[:]).astype(np.float)
X_test = np.reshape(X_test,(len(X_test),1))

Y_test = np.array(gen[:]).astype(np.float)
